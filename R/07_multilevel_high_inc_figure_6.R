################################################################################
# 07_multilevel_high_inc_figure_6.R
#
# Purpose:
#   Multilevel models for high-income mismatch:
#     delta_hishare = programme share high income − neighbourhood share high income
#
# Inputs:
#   - data/final/sthlm_95_23_final_new.csv
#
# Outputs (optional):
#   - out_tables/high_inc_tables/   (ICC + model tables)
#   - out_plots_multilevel/         (prediction plots)
#   - save_filer/high_inc/          (RDS checkpoints)
#
# Notes:
...
out_dir_tbl <- "out_tables/high_inc_tables/"
out_dir_plt <- "out_plots_multilevel/"
out_dir_rds <- "save_filer/high_inc/"
...
vars_keep <- c(
  ...
  "neigh_share_highinc","neigh_share_foreign","neigh_share_lowedu","neigh_share_lowinc",
...
df <- df %>%
  mutate(
    year = factor(year),
    delta_hishare = prog_share_highinc - neigh_share_highinc
  )
...
q <- quantile(df$neigh_share_highinc, probs = c(0, .20, .80, 1), na.rm = TRUE)

df <- df %>%
  mutate(
    neigh_highinc_tertile = cut(
      neigh_share_highinc,
      breaks = q,
      include.lowest = TRUE,
      labels = c("Low", "Medium", "High")
    ),
    neigh_highinc_tertile = factor(neigh_highinc_tertile, levels = c("Low","Medium","High"))
  )
...
m2 <- lmer(
  delta_hishare ~
    sex + foreign_back + g_lowinc + g_highinc + g_highedu + g_lowedu +
    gpa_y9 + prog_yp_hp + hman_2gr +
    neigh_share_foreign + neigh_share_lowedu + neigh_share_lowinc + neigh_highinc_tertile +
    (1 | deso_home) + (1 | new_id_2) + (1 | new_id_2:prog),
  data = df, REML = TRUE
)
...
m4 <- lmer(
  delta_hishare ~
    ...
    share_private_year + share_vocational_year +
    ratio_cohort_to_options_year + cohort_size_year +
    share_private_year * g_highinc +
    share_private_year * neigh_highinc_tertile +
    (1 | deso_home) + (1 | new_id_2) + (1 | new_id_2:prog),
  data = df, REML = TRUE,
  control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5))
)
...
## ---------------------------------------------------------------
## 7) ICC tables (total + by level) — HIGH INCOME (hishare) ####
## ---------------------------------------------------------------

ICC_STUB <- "hishare"  # high income share
...
ft <- modelsummary(
  ...
  fmt = 3,
  output = "flextable",
  title = "Table X. Multilevel models of school–neighbourhood mismatch (Δ share high income)"
)
...
save_as_docx(ft, path = file.path(out_dir_tbl, "table_hishare_stepwise_APA.docx"))
...
write.csv(tab_step, file.path(out_dir_tbl, "table_hishare_stepwise.csv"), row.names = FALSE)

## ------------------------------------------------------------
## 8c) Save checkpoints (fast reload) ####
## ------------------------------------------------------------

#saveRDS(df, file.path(out_dir_rds, "df_prepped.rds"), compress = "xz")
#saveRDS(m0, file.path(out_dir_rds, "m0_null.rds"), compress = "gzip")
#saveRDS(m1, file.path(out_dir_rds, "m1_individual.rds"), compress = "gzip")
#saveRDS(m2, file.path(out_dir_rds, "m2_neighbourhood.rds"), compress = "gzip")
#saveRDS(m3, file.path(out_dir_rds, "m3_system.rds"), compress = "gzip")
saveRDS(m4, file.path(out_dir_rds, "m4_interaction.rds"), compress = "gzip")
saveRDS(icc_tbl, file.path(out_dir_rds, "icc_hishare.rds"))
saveRDS(tab_step, file.path(out_dir_rds, "table_hishare.rds"))

## ------------------------------------------------------------
## 9) Spline model (Strategy B: no year FE) ####
## ------------------------------------------------------------
...
m4_spline <- lmer(
  delta_hishare ~
    ...
    splines::ns(share_private_year, df = 3):g_highinc +
    splines::ns(share_private_year, df = 3):neigh_highinc_tertile +
    (1 | deso_home) + (1 | new_id_2) + (1 | new_id_2:prog),
  data = df, REML = TRUE,
  control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5))
)
...
nd <- tidyr::expand_grid(
  ...
  g_highinc             = factor(c("FALSE","TRUE"), levels = levels(df$g_highinc)),
  neigh_highinc_tertile = factor(c("Low","Medium","High"), levels = levels(df$neigh_highinc_tertile))
) %>%
  mutate(
  ...
  mutate(
    ...
    g_highinc = forcats::fct_recode(
      g_highinc,
      "Not high income" = "FALSE",
      "High income"     = "TRUE"
    )
  )

p1 <- ggplot(nd, aes(x = share_private_year, y = pred, colour = g_highinc)) +
  geom_line(linewidth = 1) +
  facet_wrap(~ neigh_highinc_tertile) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Privatisation and differences in program–neighbourhood composition (high income share)",
    subtitle = "Spline predictions by high income background and neighbourhood high income context",
...
## ----------------------------------------------------------------
## 10A) Main figure: high income only (context held at Medium) ####
## ----------------------------------------------------------------

ref_neigh <- "Medium"
if (!ref_neigh %in% levels(df$neigh_highinc_tertile)) {
  ref_neigh <- levels(df$neigh_highinc_tertile)[1]
}
...
nd_main <- tidyr::expand_grid(
  ...
  mutate(
    neigh_highinc_tertile = factor(ref_neigh, levels = levels(df$neigh_highinc_tertile)),
...
p_main <- ggplot(nd_main, aes(x = share_private_year, y = pred, colour = g_highinc)) +
  geom_line(linewidth = 1) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Privatisation and differences in program–neighbourhood composition\n(high income share)",
    subtitle = paste0(
      "Spline predictions by high income background\n",
      "(neighbourhood high income-share held at: ", ref_neigh, ")"
    ),
    x = "Share of students in non-public schools",
    y = "Predicted difference between\nprogram and neighbourhood share",
    colour = "Student background"
...
make_nd_context <- function(neigh_level, context_label) {
  ...
  tidyr::expand_grid(
    ...
    mutate(
      neigh_highinc_tertile = factor(neigh_level, levels = levels(df$neigh_highinc_tertile)),
...
if (!all(need_levels %in% levels(df$neigh_highinc_tertile))) {
  stop(
    "neigh_highinc_tertile does not contain: ",
    paste(setdiff(need_levels, levels(df$neigh_highinc_tertile)), collapse = ", ")
  )
}

pred_low  <- make_nd_context("Low",  "Low share high income in neighbourhood")
pred_high <- make_nd_context("High", "High share high income in neighbourhood")
...
p_2panels <- ggplot(pred_2panels, aes(x = share_private_year, y = pred, colour = g_highinc)) +
  geom_line(linewidth = 1) +
  facet_wrap(~ context, ncol = 2) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Privatisation and differences in program–neighbourhood composition\n(high income share)",
    subtitle = "Spline predictions by student high income in low and\nhigh high-income neighbourhood contexts",
    x = "Share of students in non-public schools",
    y = "Predicted difference between\nprogram and neighbourhood share",
    colour = "Student background"
  ) +
...
save_plot_pdf_png(
  plot = p1,
  filename_stub = "figure_hishare_spline_highinc_context_3panels",
  out_dir = out_dir_plt,
...
save_plot_pdf_png(
  plot = p_main,
  filename_stub = "figure_hishare_spline_main_student_highinc",
  out_dir = out_dir_plt,
  width = 8, height = 4.5
)
...
save_plot_pdf_png(
  plot = p_2panels,
  filename_stub = "figure_hishare_spline_low_vs_high_neigh_context",
  out_dir = out_dir_plt,
  width = 8, height = 4.5
)
