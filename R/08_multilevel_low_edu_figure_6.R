################################################################################
# 08_multilevel_low_edu_figure_6.R
#
# Purpose:
#   Multilevel models for low-education mismatch:
#     delta_leshare = programme share low education − neighbourhood share low education
#
# Neighbourhood controls (per Andreas):
#   - Baseline controls: neigh_share_foreign + neigh_share_lowinc
#   - Context variable in this script: neigh_lowedu_tertile (20/80 cut)
#
# Inputs:
#   - data/final/sthlm_95_23_final_new.csv
#
# Outputs:
#   - out_tables/low_edu_tables/    (ICC + model tables)
#   - out_plots_multilevel/         (prediction plots)
#   - save_filer/low_edu/           (RDS checkpoints)
#
# Notes:
...
out_dir_tbl <- "out_tables/low_edu_tables/"
out_dir_plt <- "out_plots_multilevel/"
out_dir_rds <- "save_filer/low_edu/"
...
vars_keep <- c(
  ...
  "neigh_share_lowedu","neigh_share_foreign","neigh_share_lowinc",
...
df <- df %>%
  ...
  mutate(
    year = factor(year),
    delta_leshare = prog_share_lowedu - neigh_share_lowedu
  )
...
q <- quantile(df$neigh_share_lowedu, probs = c(0, .20, .80, 1), na.rm = TRUE)

df <- df %>%
  mutate(
    neigh_lowedu_tertile = cut(
      neigh_share_lowedu,
      breaks = q,
      include.lowest = TRUE,
      labels = c("Low","Medium","High")
    ),
    neigh_lowedu_tertile = factor(neigh_lowedu_tertile, levels = c("Low","Medium","High"))
  )
...
# Neighbourhood controls per Andreas: foreign + lowinc + (context tertile for lowedu)
m2 <- lmer(
  delta_leshare ~
    sex + foreign_back + g_lowinc + g_highinc + g_highedu + g_lowedu +
    gpa_y9 + prog_yp_hp + hman_2gr +
    neigh_share_foreign + neigh_share_lowinc + neigh_lowedu_tertile +
    (1 | deso_home) + (1 | new_id_2) + (1 | new_id_2:prog),
  data = df, REML = TRUE, control = ctrl_big
)
...
m4 <- lmer(
  delta_leshare ~
    ...
    share_private_year + share_vocational_year +
    ratio_cohort_to_options_year + cohort_size_year +
    share_private_year * g_lowedu +
    share_private_year * neigh_lowedu_tertile +
    (1 | deso_home) + (1 | new_id_2) + (1 | new_id_2:prog),
  data = df, REML = TRUE, control = ctrl_big
)
...
write.csv(icc_tbl, file.path(out_dir_tbl, "table_leshare_ICC.csv"), row.names = FALSE)

## ----------------------------------------------------------------
## 7B) ICC tables (total + by level) — LOW EDUCATION (leshare) ####
## ----------------------------------------------------------------
...
ft <- modelsummary(
  ...
  fmt = 3,
  output = "flextable",
  title = "Table X. Multilevel models of school–neighbourhood mismatch (Δ share low education)"
)
...
save_as_docx(ft, path = file.path(out_dir_tbl, "table_leshare_stepwise_APA.docx"))
...
write.csv(tab_step, file.path(out_dir_tbl, "table_leshare_stepwise.csv"), row.names = FALSE)
...
saveRDS(icc_tbl,  file.path(out_dir_rds, "icc_leshare.rds"))
saveRDS(tab_step, file.path(out_dir_rds, "table_leshare.rds"))
...
m4_spline <- lmer(
  delta_leshare ~
    ...
    splines::ns(share_private_year, df = 3):g_lowedu +
    splines::ns(share_private_year, df = 3):neigh_lowedu_tertile +
    (1 | deso_home) + (1 | new_id_2) + (1 | new_id_2:prog),
  data = df, REML = TRUE, control = ctrl_big
)
...
write.csv(tab_spline, file.path(out_dir_tbl, "table_leshare_spline.csv"), row.names = FALSE)
...
nd <- tidyr::expand_grid(
  ...
  g_lowedu             = factor(c("FALSE","TRUE"), levels = levels(df$g_lowedu)),
  neigh_lowedu_tertile = factor(c("Low","Medium","High"), levels = levels(df$neigh_lowedu_tertile))
) %>%
  mutate(
  ...
  mutate(
    ...
    g_lowedu = forcats::fct_recode(
      g_lowedu,
      "Not low education" = "FALSE",
      "Low education"     = "TRUE"
    )
  )

p1 <- ggplot(nd, aes(x = share_private_year, y = pred, colour = g_lowedu)) +
  geom_line(linewidth = 1) +
  facet_wrap(~ neigh_lowedu_tertile) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Privatisation and differences in program–neighbourhood composition (low education share)",
    subtitle = "Spline predictions by low education background and neighbourhood low education context",
...
# ----------------------------
# 10A) Main figure: student low education only (context held at Medium) ####
# ----------------------------
ref_neigh <- "Medium"
if (!ref_neigh %in% levels(df$neigh_lowedu_tertile)) {
  ref_neigh <- levels(df$neigh_lowedu_tertile)[1]
}
...
nd_main <- tidyr::expand_grid(
  ...
  mutate(
    neigh_lowedu_tertile = factor(ref_neigh, levels = levels(df$neigh_lowedu_tertile)),
...
p_main <- ggplot(nd_main, aes(x = share_private_year, y = pred, colour = g_lowedu)) +
  geom_line(linewidth = 1) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Privatisation and differences in program–neighbourhood composition\n(low education share)",
    subtitle = paste0(
      "Spline predictions by low education background\n",
      "(neighbourhood low education-share held at: ", ref_neigh, ")"
    ),
    x = "Share of students in non-public schools",
    y = "Predicted difference between\nprogram and neighbourhood share",
    colour = "Student background"
...
make_nd_context <- function(neigh_level, context_label) {
  tidyr::expand_grid(
    ...
    mutate(
      neigh_lowedu_tertile = factor(neigh_level, levels = levels(df$neigh_lowedu_tertile)),
...
if (!all(need_levels %in% levels(df$neigh_lowedu_tertile))) {
  stop(
    "neigh_lowedu_tertile does not contain: ",
    paste(setdiff(need_levels, levels(df$neigh_lowedu_tertile)), collapse = ", ")
  )
}

pred_low  <- make_nd_context("Low",  "Low share low education in neighbourhood")
pred_high <- make_nd_context("High", "High share low education in neighbourhood")
...
p_2panels <- ggplot(pred_2panels, aes(x = share_private_year, y = pred, colour = g_lowedu)) +
  geom_line(linewidth = 1) +
  facet_wrap(~ context, ncol = 2) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Privatisation and differences in program–neighbourhood composition\n(low education share)",
    subtitle = "Spline predictions by low education background in low and\nhigh low-education neighbourhood contexts",
    x = "Share of students in non-public schools",
    y = "Predicted difference between\nprogram and neighbourhood share",
    colour = "Student background"
  ) +
...
save_plot_pdf_png(
  plot = p1,
  filename_stub = "figure_leshare_spline_lowedu_context_3panels",
  out_dir = out_dir_plt,
...
save_plot_pdf_png(
  plot = p_main,
  filename_stub = "figure_leshare_spline_main_student_lowedu",
  out_dir = out_dir_plt,
  width = 8, height = 4.5
)
...
save_plot_pdf_png(
  plot = p_2panels,
  filename_stub = "figure_leshare_spline_low_vs_high_neigh_context",
  out_dir = out_dir_plt,
  width = 8, height = 4.5
)
